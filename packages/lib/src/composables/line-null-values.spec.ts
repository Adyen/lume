...
import { useLineNullValues } from './line-null-values';

import { data } from '@test/unit/mock-data';
...
describe('line-null-values.ts', () => {
  test('should return expected object', () => {
    ...
    expect(
      data[0].values.every(
        (_, index) => mixin.computedLineData.value[0].isDashed(index) === false
      )
    ).toEqual(true);
  ...
  test('should return expected object when some values are null', () => {
    ...
    expect(mixin.computedLineData.value[0].isDashed(mutatedIndex)).toEqual(
      true
    );
    expect(mixin.computedLineData.value[0]).toHaveProperty('values');
    expect(mixin.computedLineData.value[0].values[mutatedIndex]).toHaveProperty(
      'label'
    );
    expect(mixin.computedLineData.value[0].values[mutatedIndex].label).toEqual(
      'No data'
    );
    expect(mixin.computedLineData.value[0].values[lastIndex].label).toEqual(
      'No data'
    );

    const prevValue =
      mixin.computedLineData.value[0].values[mutatedIndex - 1].value;
    const nextValue =
      mixin.computedLineData.value[0].values[mutatedIndex + 1].value;
    const expectedInterpolatedValue = (prevValue + nextValue) / 2;
    expect(mixin.computedLineData.value[0].values[mutatedIndex].value).toEqual(
      expectedInterpolatedValue
    );
  });
});
