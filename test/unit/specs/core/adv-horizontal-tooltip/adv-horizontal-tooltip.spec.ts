...
const getWrapped = (
  ...
  opened = true,
  hoveredIndex = null,
  mountTargetElement = false,
  returnValue = {}
) => {
  return mount(
    {
      components: { AdvHorizontalTooltip },
      props: {
      ...
      template: `
            <div>
                ...
                <adv-horizontal-tooltip
                    ...
                    ${
                      mountTargetElement
                        ? ':target-element="$refs.targetElement"'
                        : ''
                    }
                >
    ...
    { propsData: { data, opened, mountTargetElement } }
  );
};

describe('adv-horizontal-tooltip.vue', () => {
  test('mounts component', async () => {
    ...
    const el = wrapped.find('[data-j-horizontal-tooltip]');
    expect(el.exists()).toBeTruthy();
    expect(el.attributes()['style']).toEqual('top: 0px; left: 0px;');
    expect(
      wrapped.find('[data-j-final-destination]').attributes()['hovered-index']
    ).toEqual('-1');
  });

  test('should not expose root element if opened is set to false', () => {
    const data = generateData(2, 7);
    const wrapper = mount(AdvHorizontalTooltip, {
      propsData: {
        data,
        opened: false,
      },
    ...
    expect(wrapper.find('[data-j-horizontal-tooltip]').exists()).toBeFalsy();
  });
  ...
  test('should expose the correct hovered index inside the slot', async () => {
    ...
    expect(wrapped.find('[data-j-final-destination]').text()).toEqual(
      data[0].values[0].value.toString()
    );
  });
  ...
  test('should mount the tooltip on the designated target element', async () => {
    ...
    const el = wrapped.find('[data-j-horizontal-tooltip]');
    expect(el.attributes()['style']).toEqual(
      `top: ${returnValue.y}px; left: ${returnValue.x}px;`
    );
  });
  ...
  test('should display the value of the hovered entry', () => {
    ...
    expect(wrapper.text()).toEqual(
      data[0].values[hoveredIndex].value.toString()
    );
  });
});
