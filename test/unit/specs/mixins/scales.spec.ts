...
const aboveZeroDataSet = [{ values: [{ value: 10 }, { value: 20 }, { value: 30 }, { value: 40 }] }];
const flatData = data.reduce((acc, { values }) => [
  ...acc, ...values.reduce((acc2, { value }) => [...acc2, value], [])
], [])
const min = Math.min(...flatData);
const max = Math.max(...flatData);

const getMixin = async (data, orientation: Orientation = 'horizontal', options: ChartOptions = {}) => {
  let mixin = null;

  await mount({
    template: '<div></div>',
    setup() {
      mixin = useBaseScales(ref(data), ref(labels), containerSize, ref(orientation), ref(options));
      return mixin;
    },
  });
...
describe('scales.ts', () => {
  describe('getMixin', () => {
    ...
    test('should start scale on lowest value in dataset', async () => {
      const mixin = await getMixin(aboveZeroDataSet, 'horizontal', { startOnZero: false });

      expect(mixin.xScale.value.domain()[0]).toEqual(Math.min(...aboveZeroDataSet[0].values.map(({ value }) => value)));
    })

    test('should start scale on 0', async () => {
      const mixin = await getMixin(aboveZeroDataSet, 'horizontal', { startOnZero: true });
  ...
  describe('getXByIndex', () => {
    test('should give correct x value by index', async () => {
      ...
      expect(getXByIndex(yScale.value, index)).toEqual(yScale.value.bandwidth() / 2 + yScale.value(yScale.value.domain()[index]));
    })
  });
