...
```ts
import { LumeSingleBarChart } from 'lume/charts';
```
...
```html
<template>
  <lume-single-bar-chart :data="data" :labels="labels" />
</template>
...
```html
<template>
  <lume-single-bar-chart :data="data" :labels="labels" :orientation="horizontal" />
</template>
